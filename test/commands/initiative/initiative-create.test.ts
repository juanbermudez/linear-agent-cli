...
await snapshotTest({
  name: "Initiative Create Command - Missing Required Name",
  meta: import.meta,
  colors: false,
  args: [
    "--owner",
...
// Test error when owner not found
await snapshotTest({
  name: "Initiative Create Command - Owner Not Found",
  meta: import.meta,
  colors: false,
  args: [
    "--name",
  ...
  async fn() {
    const { cleanup } = await setupMockLinearServer([
      {
        queryName: "LookupUser",
        variables: { input: "nonexistent@company.com" },
        response: {
          data: {
            users: {
              nodes: [],
    ...
    try {
      ...
      // Expected to fail - owner not found
    } finally {
      await cleanup()
    }
  },
