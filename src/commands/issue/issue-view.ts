...
export const viewCommand = new Command()
  ...
  .action(async (options, issueId) => {
    ...
    if (metadata.identifier) {
      metadataMarkdown += `**Issue:** ${metadata.identifier}\n`
    }
    if (metadata.state) {
      metadataMarkdown += `**Status:** ${metadata.state.name}\n`
    }
    if (metadata.team) {
      metadataMarkdown += `**Team:** ${metadata.team.key}\n`
    }
    ...
    if (metadata.dueDate) {
      metadataMarkdown += `**Due:** ${metadata.dueDate}\n`
    }
    if (metadata.project) {
